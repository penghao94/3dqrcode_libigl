...
bool qrcode::direction_light(igl::viewer::Viewer & viewer, Engine * engine, Eigen::Vector4d & position, Eigen::Vector4d& position1, Eigen::MatrixXd & D, Eigen::Matrix4f & mode,float zoom, int wht_num, int mul, int ext, Eigen::MatrixXd & V_uncrv, Eigen::MatrixXi & F_qr, Eigen::MatrixXd & C_qr, Eigen::MatrixXi & E_qr, Eigen::MatrixXd & H_qr, Eigen::MatrixXf & Src, Eigen::MatrixXf & Dir, std::vector<Eigen::MatrixXd>& th, Eigen::MatrixXd & V_pxl, Eigen::MatrixXd & V_rest, Eigen::MatrixXi & F_rest, Eigen::MatrixXi & E_rest, Eigen::MatrixXd & V_fin, Eigen::MatrixXi & F_fin)
{
	using namespace std;
	...
	VectorXd dir(3),dir1(3), pos(3),pos1(3);// user design position

	dir << position(0), position(1), position(2);
	dir1 << position1(0), position1(1), position1(2);
	
	pos = centr + position(3)*zoom*dir;
	pos1 = centr + position1(3)*zoom*dir1;
	...
	pos1.conservativeResize(4);

	pos1.col(3).setConstant(1);

	pos1 = (model*(pos1.cast<float>())).cast<double>().block(0, 0, 3, 1);

	dir1 = (pos1 - centr).normalized();
	igl::embree::EmbreeIntersector ei;	//rays

	VectorXd Bmap, Wmap;// is_light result of black and white pixel;
	VectorXd Bmap1;// is_light result of black and white pixel;
	...
	qrcode::is_light(ei, V_fin, F_fin, pos1, Pb, Bmap1);
	...
	const auto range1 = [&dir1, &pos1](RowVectorXd des)->bool {
		VectorXd d = (pos1 - des.transpose()).normalized();
		if (sqrt(1.0 - d.dot(dir1)) > 1.0 / sqrt(101.0))
			return false;
		else
			return true;
	};
	...
	for (int i = 0; i < BW.rows() - 1; i++) {
		for (int j = 0; j < BW.cols() - 1; j++) {
			...
			if (BW(i, j) > 0) {
				if (((i >= 20 && i < 30) || (i >= 60 && i<70)) && j >= 30 && j<40) {
					for (int m = 0; m < scale; m++) {
						for (int n = 0; n < scale; n++) {

							/*Determine if Qr code is in the range of direction light*/
							if (!range1(V_pxl.row((i*scale + m)*th[0].cols() + j*scale + n))) {
							...
							if (Bmap1(bindex) != 0) {
								th_crv(i*scale + m, j*scale + n) += 0.4*step;
								stop = false;
							}
							bindex++;
						}
	...
	while (!stop) {
		...
		Eigen::MatrixXd bbw = th_crv.block(30, 30, 30, 30);
		double deep = bbw.maxCoeff();
		bbw.setConstant(deep);
		th_crv.block(60, 60, 30, 30) = bbw;
		/*Qr code simulation in ambient light condition*/
