...
int qrcode::img_to_sep_mesh(igl::viewer::Viewer & viewer, Eigen::MatrixXd & V, Eigen::MatrixXi & F, Eigen::MatrixXd & D, int scale, int acc, Eigen::MatrixXi & fid, Eigen::MatrixXd & _V, Eigen::MatrixXi & _F, Eigen::MatrixXd & _C, Eigen::MatrixXi & _E, Eigen::MatrixXd & _H, Eigen::MatrixXf &Src, Eigen::MatrixXf &Dir, std::vector<Eigen::MatrixXd> &L, Eigen::MatrixXd &V_pxl)
{
	...
	fid.resize(row + 2 *tr* mul, col + 2*tr * mul);
	L[0].resize(row, col);
	L[1].resize(row, col);
	...
	*/
	for (int i = -tr*mul; i < row + tr*mul; i++) {
		for (int j = -tr*mul; j < col + tr*mul; j++) {
			...
			bool shoot = unproject_to_mesh(Eigen::Vector2f(x, y), viewer.core.view*viewer.core.model,
				viewer.core.proj, viewer.core.viewport, V, F, s, dir, fid(i + tr*mul, j + tr*mul), _uv, t);
			if (i >= 0 && j >= 0 && i < row&&j < col) {
				v0 = V.row(F(fid(i + tr*mul, j + tr*mul), 0));
				v1 = V.row(F(fid(i + tr*mul, j + tr*mul), 1));
				v2 = V.row(F(fid(i + tr*mul, j + tr*mul), 2));
