	...
	for (int i = 0; i < D.rows(); i++) {
		for (int j = 0; j < D.cols(); j++) {
			if (D(i, j) == 1.0&&i<D.rows()-1&&j<D.rows()-1) {
				...
				_V.row(4 * b_idx + 1 + wht_num) <<( Src.row((m+mul)*col+n)+ (T(m+mul, n) + addT(i + 1, j))*Dir.row((m + mul)*col + n)).cast<double>();
				_V.row(4 * b_idx + 2 + wht_num) << (Src.row(m*col + n+mul)+ (T(m, n+mul) + addT(i, j+1))*Dir.row(m*col + n + mul)).cast<double>();
				_V.row(4 * b_idx + 3 + wht_num) <<( Src.row((m + mul)*col + n+mul) + (T(m+mul,n+mul) + addT(i+1, j+1))*Dir.row((m + mul)*col + n + mul)).cast<double>();
