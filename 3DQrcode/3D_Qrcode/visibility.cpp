			...
			if (BW(i, j) >0) {
				for (int m = 0; m < scale; m++) {
					for (int n = 0; n < scale; n++) {
						...
						//to get qrcode bound verticals
						MatrixXd V_qr1(0, 3);
						/*if (th(x, y - 1) == th(x - 1, y)) {
							V_qr1.conservativeResize(V_qr1.rows() + 1,3);
							V_qr1.row(V_qr1.rows() - 1) << Src.row(x*th.cols() + y).cast<double>() + Dir.row(x*th.cols() + y).cast<double>()*(th(x, y) + th_crv(x, y - 1));
						}
						else {
							V_qr1.conservativeResize(V_qr1.rows() + 2, 3);
							V_qr1.row(V_qr1.rows() - 2) << Src.row(x*th.cols() + y).cast<double>() + Dir.row(x*th.cols() + y).cast<double>()*(th(x, y) + th_crv(x, y - 1));
							V_qr1.row(V_qr1.rows() - 1) << Src.row(x*th.cols() + y).cast<double>() + Dir.row(x*th.cols() + y).cast<double>()*(th(x, y) + th_crv(x - 1, y));
						}
						if (th(x, y + 1) == th(x - 1, y)) {
							V_qr1.conservativeResize(V_qr1.rows() + 1, 3);
							V_qr1.row(V_qr1.rows() - 1) << Src.row(x*th.cols() + y + 1).cast<double>() + Dir.row(x*th.cols() + y + 1).cast<double>()*(th(x, y + 1) + th_crv(x - 1, y));
						}
						else {
							V_qr1.conservativeResize(V_qr1.rows() + 2, 3);
							V_qr1.row(V_qr1.rows() - 2) << Src.row(x*th.cols() + y + 1).cast<double>() + Dir.row(x*th.cols() + y + 1).cast<double>()*(th(x, y + 1) + th_crv(x - 1, y));
							V_qr1.row(V_qr1.rows() - 1) << Src.row(x*th.cols() + y + 1).cast<double>() + Dir.row(x*th.cols() + y + 1).cast<double>()*(th(x, y + 1) + th_crv(x, y + 1));
						}
						if (th(x, y + 1) == th(x + 1, y)) {
							V_qr1.conservativeResize(V_qr1.rows() + 1, 3);
							V_qr1.row(V_qr1.rows() - 1) << Src.row((x + 1)*th.cols() + y + 1).cast<double>() + Dir.row((x + 1)*th.cols() + y + 1).cast<double>()*(th(x + 1, y + 1) + th_crv(x, y + 1));
						}
						else {
							V_qr1.conservativeResize(V_qr1.rows() + 2, 3);
							V_qr1.row(V_qr1.rows() - 2) << Src.row((x + 1)*th.cols() + y + 1).cast<double>() + Dir.row((x + 1)*th.cols() + y + 1).cast<double>()*(th(x + 1, y + 1) + th_crv(x, y + 1));
							V_qr1.row(V_qr1.rows() - 1) << Src.row((x + 1)*th.cols() + y + 1).cast<double>() + Dir.row((x + 1)*th.cols() + y + 1).cast<double>()*(th(x + 1, y + 1) + th_crv(x + 1, y));
						}
						if (th(x, y - 1) == th(x + 1, y)) {
							V_qr1.conservativeResize(V_qr1.rows() + 1, 3);
							V_qr1.row(V_qr1.rows() - 1) << Src.row((x + 1)*th.cols() + y).cast<double>() + Dir.row((x + 1)*th.cols() + y).cast<double>()*(th(x + 1, y) + th_crv(x + 1, y));
						}
						else {
							V_qr1.conservativeResize(V_qr1.rows() + 2, 3);
							V_qr1.row(V_qr1.rows() - 1) << Src.row((x + 1)*th.cols() + y).cast<double>() + Dir.row((x + 1)*th.cols() + y).cast<double>()*(th(x + 1, y) + th_crv(x + 1, y));
							V_qr1.row(V_qr1.rows() - 1) << Src.row((x + 1)*th.cols() + y).cast<double>() + Dir.row((x + 1)*th.cols() + y).cast<double>()*(th(x + 1, y) + th_crv(x, y - 1));
						}*/
