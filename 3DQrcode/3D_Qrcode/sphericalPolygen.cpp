...
double qrcode::multiIntersection(Engine *engine, Eigen::MatrixXd & V_qr, std::vector<Eigen::MatrixXd>& V_md, std::vector<Eigen::MatrixXd>&V_flag, Eigen::MatrixXd & rot, Eigen::MatrixXd & dir)
{
	...
	*/
	...
	for (int i = 0; i < V_qr.rows(); i++) {
		...
		if (y1 >= 0) {
			if (x1 >= 0) {
				polygons[0].outer().push_back(Point_c(asin(y1 / w1), asin(z1)));
				...
				if (a(0) < 0) {
					...
					//temp.row(index) << polygons[0].outer().back().x(), polygons[0].outer().back().y(), 0;
					//index++;
					polygons[0].outer().push_back(Point_c(pi - asin(y1 / w1), asin(z1)));
				}
				else
					polygons[0].outer().push_back(Point_c(pi - asin(y1 / w1), asin(z1)));
			...
			if (y >= 0) {
				if (x >= 0) {
					...
					//	temp.conservativeResize(temp.rows() + 4, 3);
						...
						polygons[i + 1].outer().push_back(Point_c(-pi, f));
...
#pragma omp parallel for schedule(dynamic)
	for (int i = 0; i < V_md.size(); i++) {
		...
		for (int j = 0; j < V_md[i].rows(); j++) {
			...
			if (y >= 0) {
				if (x >= 0) {
					polygons[i].outer().push_back(Point_c(asin(y / w), asin(z)));
					c << asin(y / w), asin(z);
				}
				else {
					if (c(0) < 0) {
						...
						//	temp.conservativeResize(temp.rows() + 4, 3);
						f = (asin(z) - c(1)) / (2 * pi - pi + asin(y / w) + c(0))*(pi + c(0)) + c(1);
						polygons[i].outer().push_back(Point_c(-pi, f));
						//temp.row(index) << polygons[i+1].outer().back().x(), polygons[i + 1].outer().back().y(), 0;
						//	index++;
						polygons[i].outer().push_back(Point_c(-pi, 1.57079632));
						//	temp.row(index) << polygons[i + 1].outer().back().x(), polygons[i + 1].outer().back().y(), 0;
						//	index++;
						polygons[i].outer().push_back(Point_c(pi, 1.57079632));
						//	temp.row(index) << polygons[i + 1].outer().back().x(), polygons[i + 1].outer().back().y(), 0;
						...
						//	temp.row(index) << polygons[i + 1].outer().back().x(), polygons[i + 1].outer().back().y(), 0;
						//	index++;
						polygons[i].outer().push_back(Point_c(pi - asin(y / w), asin(z)));
					}
					else
						polygons[i].outer().push_back(Point_c(pi - asin(y / w), asin(z)));
					c << pi - asin(y / w), asin(z);
